...
public class BidderAgent extends Agent {
    ...
    public void setup() {
        ...
        if (Arglenght <= 0) {
            ...
            ags = (AgentState) Arguments;
            ags.setMyAid(this.getAID());
            ags.setMyname(this.getLocalName());
            try {
                getregister1(this.getLocalName(), ags.getMyAid(), ags.getCloneAid());
            } catch (IOException ex) {
                Logger.getLogger(BidderAgent.class.getName()).log(Level.SEVERE, null, ex);
            }
          // ags.setCloneAid(childaid);
        ...
        while (flagloop) {
            ...
            try {
                ...
                ac.addReceiver(ags.getCloneAid());
                send(ac);
            ...
            if (acl != null && acl.getPerformative() == ACLMessage.INFORM) {
                if (acl.getContent().equals("BID")) {
                    ...
                    if (mybid != -1 & mybid != -255 & !(mybid <= 0) & mybid != -101) {
                        System.out.println(this.getLocalName()
                                + "::  my bid is $" + mybid +"  Where total amount I have is "+ags.getMymoney()+ "And my last bid was "+ ags.getMylastbid());
                        ags.setMylastbid(mybid);
                        ...
                        send(reply);
                        ACLMessage acltemp = new ACLMessage(ACLMessage.INFORM_REF);
                        try {
                ...
                Object[] StatesArgumetns = new Object[10];
                  ...
                  StatesArgumetns[0] = (Object) ags;
                try {
                    this.getContainerController().createNewAgent(acl.getContent(), "jadeacutonsystem.BidderAgent", StatesArgumetns).start();
    ...
    public int calculatebid(int moneyinhand, int higestbid) {
        ...
        if (higestbid < moneyinhand & higestbid != 0) {
            ...
            if (mybid > moneyinhand) {
                mybid = new Double( 0.5 * higestbid).intValue();
                mybid = mybid + higestbid;
                if (mybid > moneyinhand) {
                    mybid = -1;
                }
